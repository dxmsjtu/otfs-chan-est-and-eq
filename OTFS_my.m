...
clear all; close all; addpath('functions');  addpath('classes');  clear variables; rng('default');
% rng('shuffle');
...
% ----- Channel -----
channelModel = 'EVA'; fprintf("Channel profile: %s\n", channelModel);
%---- Channel Coding ----
channelCoding = 'LTE';
% channelCoding = 'None';
...
switch channelCoding
    case 'None'
        ...
        % Symbol Mapper
        hSymbolMapper = QamMapperV2('ModOrder', modOrder,'InputType', 'bit','OutputType', 'bit', ...  % 'bit' for uncoded, 'llr' for coded
            'UnitAveragePower', true,'LLROverflowPrevention', true);
    case 'LTE'
        % Bit Generator
        hBitGenerator = RandomBitGenerator('DataLength', tbs);
        % Symbol Mapper
        hSymbolMapper = QamMapperV2('ModOrder', modOrder,'InputType', 'bit','OutputType', 'llr', ...  % 'bit' for uncoded, 'llr' for coded
            'UnitAveragePower', true,'LLROverflowPrevention', true);
        % Symbol Mapper for uncoded
        hSymbolMapperUncoded = QamMapperV2( ...
            'ModOrder', modOrder, 'InputType', 'bit', 'OutputType', 'bit', ...  % 'bit' for uncoded, 'llr' for coded
            'UnitAveragePower', true, 'LLROverflowPrevention', true);
        % LTE Channel Coder
        hChannelCoder = LteChannelCoder( ...
            'TurboDecodingNumIterations', turboDecodingNumIterations, ...
            'ModOrder', modOrder, 'NumLayers', 1, 'OutputLength', nBitsPerBlock, ...
            'LinkDirection', 'Downlink', 'RedundancyVersion', 0, 'TBS', tbs);
        % Scrambler
...
hFading = SoSBasedChannel( ...  % based on in-house implementation
    'ChannelModel', channelModel, 'RMSDelaySpread', delaySpread_ns, 'SamplingRate', samplingRate, ...
    'DopplerFrequency', fDoppler, 'NumTxAntennas', nTxAntennas,'NumRxAntennas', nRxAntennas, ...
    'TxCorrMatrix', txCorrMat, 'RxCorrMatrix', rxCorrMat,'OutputCIR', true, 'CyclicPrefix', nCyclicPrefix, ...
    'FFTSize', nFft,  'ImpulseSource', 'Input', 'SequentialOrRandomChannel', 'Sequential','FDFMethod', 'ApplyFDFToPathParameters');
% AWGN
...
charCount = 49+17;
for snrdb = snrRange
    rng('default');    rng(106);    fprintf('\nSNR = %2d dB \n', snrdb);
    snrIndex = find(snrRange==snrdb);    snr = 10^(snrdb/10);
